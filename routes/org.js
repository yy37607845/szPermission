...
router.get('/getAllOrg', function (req, res, next) {
  ...
  var sql = 'select objname as orgName, objno as orgNo from sz_org  order by dsporder ' 
...
router.post('/queryOrgByName', function(req, res, next){
  ...
  var sql = 'select objname as orgName, objno as orgNo from sz_org a ' +
  " where objno like '%" + orgName + "%' or objname like '%" + orgName + "%'" ;
  query(sql, function (err, vals, fields) {
    if (err) {
      console.log(err);
    } else {
